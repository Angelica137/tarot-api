...
def test_get_or_create_user():
    with patch("app.services.user_service.User") as MockUser, patch(
        "app.services.user_service.db.session.add"
    ) as mock_add, patch("app.services.user_service.db.session.commit") as mock_commit:
        # Setup mock user
        ...
        with patch("app.services.user_service.User.query") as mock_query:
            ...
            MockUser.assert_called_once_with(
                auth0_user_id="auth0|123", name="Test User", email="test@example.com"
            )
