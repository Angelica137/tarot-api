...
def test_new_card(test_app, sample_card_data):
    ...
    assert json.loads(new_card.fortune_telling)[0] == "Watch for new projects \
            and new beginnings"
    assert json.loads(new_card.keywords)[0] == "freedom"
    assert json.loads(new_card.meanings)["light"][0] == "Freeing yourself \
            from limitation"
    ...
    assert json.loads(new_card.questions_to_ask)[0] == "What would I do if I \
            felt free to take a leap?"
